...
def get_all_bookmarked(url, data, token_response)
  response = JSON.parse(bookmarked_tweets(url, token_response).body)

  if response['meta']['next_token'] then
    data['data'] ||= []
    data['meta'] ||= {}
    ...
    new_data = {
      ...
      'meta' => {
        ...
        'result_count' => data['meta']['result_count'] + response['meta']['result_count']
      }
    }

    next_token = new_data['meta']['next_token']
    next_bookmarks_url = "https://api.twitter.com/2/users/#{$user_id}/bookmarks?max_results=#{$max_results}&pagination_token=#{next_token}"
    data = get_all_bookmarked(next_bookmarks_url, new_data, token_response)
  else
    data['data'] ||= []
    response['data'] ||= []
    data['meta'] ||= {}
...
$max_results = 5
initial_bookmarks_url = "https://api.twitter.com/2/users/#{$user_id}/bookmarks?max_results=#{$max_results}"
response = get_all_bookmarked(initial_bookmarks_url, {}, token_response)

File.open('bookmark.json', 'w') do |file|
